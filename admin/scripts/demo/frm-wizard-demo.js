...
$( '#panelWizard2' ).bootstrapWizard({
	...
	$('#panelWizard').find('.submit').on( 'click', function(e){
		e.preventDefault();
		$( '#alert-submit' ).remove();
		$('#panelWizard .panel-body').prepend( '<div id="alert-submit" class="alert alert-info no-padding"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> <strong>Wahoo!</strong> You are submited!</div>' );
	});

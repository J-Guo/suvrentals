(function(){
  ...
  var suburbs = new Bloodhound({
    ...
    limit: 10,
    prefetch: {
      url: '_includes/australia_suburbs.json',
...
// passing in `null` for the `options` arguments will result in the default
// options being used
$('#typeahead-prefetches2').typeahead(null, {
    name: 'suburbs',
    displayKey: 'name',
    // `ttAdapter` wraps the suggestion engine in an adapter that
    // is compatible with the typeahead jQuery plugin
  ...
  .on('hide.daterangepicker', function(e, picker){
    ...
    //THis following functions for calculate fare in Booking_form.php. Line number 232.

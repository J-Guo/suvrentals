...
// Check connection
    ...
    function viewBookedRecords($get_bid,$iDel,$iRet){

        if(($iDel == 0) &&($iRet == 0))
        {
            $selBooking = mysqli_query($this->connection,"SELECT * FROM tblbooking INNER JOIN tblcustomer ON tblbooking.mobile = tblcustomer.mobile JOIN tblvehicle ON tblbooking.vrego = tblvehicle.vrego JOIN tblpayment ON tblbooking.bid = tblpayment.bid WHERE tblbooking.bid = '$get_bid'");
        ...
        }elseif(($iDel != 0) &&($iRet !=1)){
            $selBooking = mysqli_query($this->connection,"SELECT * FROM tblbooking INNER JOIN tbldelivery ON tblbooking.bid = tbldelivery.bid JOIN tblcustomer ON tblbooking.mobile = tblcustomer.mobile JOIN tblvehicle ON tblbooking.vrego = tblvehicle.vrego JOIN tblpayment ON tblbooking.bid = tblpayment.bid WHERE tblbooking.bid = '$get_bid'");
        }elseif(($iDel != 1) &&($iRet != 0)){
            $selBooking = mysqli_query($this->connection,"SELECT * FROM tblbooking INNER JOIN tblreturn ON tblbooking.bid = tblreturn.bid JOIN tblcustomer ON tblbooking.mobile = tblcustomer.mobile JOIN tblvehicle ON tblbooking.vrego = tblvehicle.vrego JOIN tblpayment ON tblbooking.bid = tblpayment.bid WHERE tblbooking.bid = '$get_bid'");
        }elseif(($iDel == 1) &&($iRet == 1)){
            //$selBooking = mysqli_query($this->connection,"SELECT * FROM tblbooking INNER JOIN tblcustomer ON tblbooking.mobile = tblcustomer.mobile JOIN tblvehicle ON tblbooking.vrego = tblvehicle.vrego JOIN tblpayment ON tblbooking.bid = tblpayment.bid WHERE tblbooking.bid = '$get_bid'");
            $selBooking = mysqli_query($this->connection,"SELECT * FROM tblbooking INNER JOIN tbldelivery ON tblbooking.bid = tbldelivery.bid JOIN tblreturn ON tblbooking.bid = tblreturn.bid JOIN tblcustomer ON tblbooking.mobile = tblcustomer.mobile JOIN tblvehicle ON tblbooking.vrego = tblvehicle.vrego JOIN tblpayment ON tblbooking.bid = tblpayment.bid WHERE tblbooking.bid = '$get_bid'");
        }
        //var_dump($selBooking);
    ...
    function  chkDeliRec($get_bid){
        $chkDeliver= mysqli_query($this->connection,"SELECT * FROM tbldelivery WHERE bid = '$get_bid'");
        //var_dump($chkDeliver);
        $rowCheck = mysqli_num_rows($chkDeliver);
        //var_dump($rowCheck);
        return $rowCheck;
    }

    function chkRetnRec($get_bid){
        $chkReturn= mysqli_query($this->connection,"SELECT * FROM tblreturn WHERE bid = '$get_bid'");
        //var_dump($selBooking);
        $rowCheck = mysqli_num_rows($chkReturn);
        //var_dump($rowSel);s
        return $rowCheck;
    }
    ...
    function delDelivery($get_bid){
        $sql = "DELETE FROM tbldelivery WHERE bid = '$get_bid'";
        $sqlQuery = mysqli_query($this->connection,$sql);
        //var_dump($sqlQuery);
    }
...
//            }
    ...
    /**
     * Get all the cars details in DB
     */
    function getVehicels(){
        ...
        while($row = mysqli_fetch_array($results,MYSQLI_ASSOC)){
            ...
            //check the date range of booked vehicel from booking table
            $checkSql = "SELECT * FROM tblbooking WHERE vrego = '$vrego'";
            $checkResults = mysqli_query($this->connection,$checkSql);

            while($checkRow = mysqli_fetch_assoc($checkResults)){
                $delivery_date = $checkRow['delivery_date'];
                $delivery_time = $checkRow['delivery_time'];
                $delivery = $delivery_date." ".$delivery_time;
                $drop_date = $checkRow['drop_date'];
                $drop_time = $checkRow['drop_time'];
                $drop = $drop_date." ".$drop_time;

                $occupiedDate[] = ['delivery' => $delivery,'drop' => $drop];
            }
//            var_dump($occupiedDate);
            $vehicelsDate[] = ['vrego'=>$vrego,'vrate' =>$vrate,'occupiedDate' => $occupiedDate];
        }

        return json_encode($vehicelsDate);

    }


    //generate a SUV information for SMS to send
    public function getSuvName($vrego){

        $sql = "SELECT * FROM tblvehicle WHERE vrego = '$vrego'";
    ...
    //generate a SUV car make inforamtion for Email to send
    public function getSuvMake($vrego){
        ...
        $suvMake = $row['car_make'];

        return $suvMake;
    }

    //generate a SUV car model inforamtion for Email to send
    public function getSuvModel($vrego){
    ...
    function deleteBooking($bid){
        ...
        $sql = "DELETE FROM tblbooking WHERE bid='$bid'";
        //var_dump($sql);
        return $sqlQuery = mysqli_query($this->connection,$sql);
    }
